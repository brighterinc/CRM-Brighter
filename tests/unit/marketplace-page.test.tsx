/**
 * Mesmo padrão de `tests/unit/outreach-page.test.tsx`: mocka autenticação,
 * organização ativa, branding e Module Engine, e chama a página (Server
 * Component async) diretamente com `renderToStaticMarkup` — sem banco, sem
 * servidor, sem Next.js real.
 */
...
import MarketplaceSettingsPage from "@/app/app/settings/modulos-licencas/page";
...
describe("MarketplaceSettingsPage", () => {
  it("monta sem erro, mostra catálogo e disclaimer de somente-leitura", async () => {
    vi.mocked(getDeploymentPlan).mockReturnValue("dedicated");
    mockModules(["core.crm", "core.contacts", "channel.whatsapp", "ai.agents"]);

    const jsx = await MarketplaceSettingsPage();
    const html = renderToStaticMarkup(jsx);

    expect(html).toContain("Módulos e licenças");
    expect(html).toContain("Simulação disponível via CLI");
    expect(html).toContain("pnpm marketplace:summary");
  ...
  it("agente não-admin é redirecionado (403), nunca vê o painel", async () => {
    ...
    vi.mocked(getDeploymentPlan).mockReturnValue("dedicated");
    mockModules(["core.crm"]);

    await expect(MarketplaceSettingsPage()).rejects.toThrow(/redirect/i);
  });
  ...
  it("nunca mostra botão de ativação/compra real", async () => {
    vi.mocked(getDeploymentPlan).mockReturnValue("lite");
    mockModules(["core.crm", "core.contacts"]);

    const jsx = await MarketplaceSettingsPage();
    const html = renderToStaticMarkup(jsx);
    expect(html).not.toMatch(/ativar agora|comprar agora|checkout/i);
  });
});
