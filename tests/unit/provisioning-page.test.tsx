/**
 * Mesmo padrão de `tests/unit/tenant-operacao-page.test.tsx`: mocka
...
import ProvisionamentoSettingsPage from "@/app/app/settings/provisionamento/page";
...
describe("ProvisionamentoSettingsPage", () => {
  it("cenário admin: monta sem erro, mostra plano de execução e nunca segredo", async () => {
    vi.mocked(getDeploymentPlan).mockReturnValue("dedicated");
    mockModules(["core.contacts", "core.pipeline", "channel.whatsapp"]);

    const jsx = await ProvisionamentoSettingsPage();
    const html = renderToStaticMarkup(jsx);

    expect(html).toContain("Provisionamento");
    expect(html).toContain("Plano de execução");
    expect(html).toContain("Simulação disponível via CLI");
    expect(html).not.toContain("Brighter Provisioning Engine");
  ...
  it("agente não-admin é redirecionado (403), nunca vê o painel", async () => {
    ...
    await expect(ProvisionamentoSettingsPage()).rejects.toThrow(/redirect/i);
  });
});
