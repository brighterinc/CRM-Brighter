...
import ProvisioningAdaptersSettingsPage from "@/app/app/settings/provisioning-adapters/page";
...
describe("ProvisioningAdaptersSettingsPage", () => {
  it("cenário admin: monta sem erro, mostra cobertura por provider e nunca segredo", async () => {
    vi.mocked(getDeploymentPlan).mockReturnValue("dedicated");
    mockModules(["core.contacts", "core.pipeline", "channel.whatsapp"]);

    const jsx = await ProvisioningAdaptersSettingsPage();
    const html = renderToStaticMarkup(jsx);

    expect(html).toContain("Adaptadores de provisionamento");
    expect(html).toContain("Cobertura por provider");
    expect(html).toContain("Simulação disponível via CLI");
    expect(html).toContain("pnpm provisioning:adapters");
    expect(html).not.toMatch(/SUPABASE_SERVICE_ROLE_KEY=\S/);
    expect(html).not.toMatch(/serviceRoleKey/i);
  });

  it("nunca mostra botão de execução real", async () => {
    vi.mocked(getDeploymentPlan).mockReturnValue("pro");
    mockModules(["core.contacts", "core.pipeline"]);

    const jsx = await ProvisioningAdaptersSettingsPage();
    const html = renderToStaticMarkup(jsx);
    expect(html).not.toMatch(/executar (agora|real)/i);
  });
  ...
  it("agente não-admin é redirecionado (403), nunca vê o painel", async () => {
    ...
    await expect(ProvisioningAdaptersSettingsPage()).rejects.toThrow(/redirect/i);
  });
