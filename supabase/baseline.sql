...
-- ---- control plane persistence + credentials vault (migration 0098) ----
...
-- in-memory: lib/tenants, lib/control-plane, lib/deployment (histórico de
-- manifesto), lib/provisioning (runs/steps), lib/provisioning-adapters
-- (conexões de provider) — mais o Credentials Vault, que é NOVO nesta fase.
--
-- Nenhuma das 8 tabelas tem organization_id: Control Plane é a Brighter
-- observando de FORA todas as instalações White Label — mesma classe de
-- tabela que incidents (migration 0021), não de contacts/crm_leads. RLS
-- segue o padrão de incidents: LIGADA, restrita a fn_is_platform_admin().
-- control_plane_secret_references (Credentials Vault) NUNCA guarda valor de
-- segredo — só reference/type/provider/vault_provider/vault_key/version/
-- status. Racional completo no arquivo da migration.
